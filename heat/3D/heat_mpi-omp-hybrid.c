...
int main ( int argc, char** argv )
{
  ...
  /* //////////// */
  /* int     thread_support_required=MPI_THREAD_FUNNELED, */
  /*         thread_support_provided; */
  /* int is_thread_main; */

  /* MPI_Init_thread (&argc, &argv, thread_support_required, &thread_support_provided); */

  /* MPI_Is_thread_main(&is_thread_main); */

  /* if (is_thread_main) { */
  /*   printf("Support level required is %d\n", thread_support_required); */
  /*   printf("Support level provided is %d of %d %d %d %d\n", */
  /* 	   thread_support_provided, */
  /* 	   MPI_THREAD_SINGLE, */
  /* 	   MPI_THREAD_FUNNELED, */
  /* 	   MPI_THREAD_SERIALIZED, */
  /* 	   MPI_THREAD_MULTIPLE); */
  /*   if (thread_support_required < thread_support_provided) { */
  /*     printf("Support level provided insufficient"); */
  /*     //exit */
  /*   } */
  /* } */
  /* //////////// */
  ...
  # pragma omp parallel for default(none) \
               shared(I,J,Kp,k1, dx,dy,dz, wnx,wny,wnz, pi, x,y,z,u)	\
               private(i,j,k,s)
  for (k=1; k<=Kp; k++) {
    for (j=0; j<J; j++) {
      for (i=0; i<I; i++) {
  ...
  # pragma omp parallel default(none) \
    shared(N,Nout, I,J,Kp,k1,k2, u,uo,u3, nu, ip,ip_lo,ip_hi, request,status) \
  ...
  # pragma omp parallel for default(none) \
               shared(I,J,Kp, x,y,z,u, fac,wnx,wny,wnz, pi) \
               private(i,j,k, s, de) \
               reduction(+:rmsp)
  for (k=1; k<=Kp; k++) {
    for (j=0; j<J; j++) {
