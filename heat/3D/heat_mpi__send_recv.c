...
int main ( int argc, char** argv )
{
  ...
  for (n=0; n<N; n++) {
    ...
    // blocking send halo data
    MPI_Send (&u[I*J    ],    I*J, MPI_DOUBLE, ip_lo, 0, MPI_COMM_WORLD);
    MPI_Send (&u[I*J* Kp],    I*J, MPI_DOUBLE, ip_hi, 0, MPI_COMM_WORLD);

    // blocking receive halo data
    MPI_Recv (&u[0],          I*J, MPI_DOUBLE, ip_lo, 0, MPI_COMM_WORLD, &status);
    MPI_Recv (&u[I*J*(Kp+1)], I*J, MPI_DOUBLE, ip_hi, 0, MPI_COMM_WORLD, &status);
