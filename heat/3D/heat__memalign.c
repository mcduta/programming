...
int main ( int argc, char** argv )
{
  ...
  double  dx,dy,dz, dt,         // discretisation steps
          ...
          fac;
  // walltime
  clock_t wtime_sol,            // solution time
 ...
 // scheme parameter (<= 1/6 for stability)
  ...
  // allocate memory
  x  = (double *) aligned_alloc( MEM_ALLOC_ALIGNMENT, I*J*K * sizeof (double) );
  y  = (double *) aligned_alloc( MEM_ALLOC_ALIGNMENT, I*J*K * sizeof (double) );
  z  = (double *) aligned_alloc( MEM_ALLOC_ALIGNMENT, I*J*K * sizeof (double) );
  u1 = (double *) aligned_alloc( MEM_ALLOC_ALIGNMENT, I*J*K * sizeof (double) );
  u2 = (double *) aligned_alloc( MEM_ALLOC_ALIGNMENT, I*J*K * sizeof (double) );
/*
...
*/
  ...
  // stop total time
  wtime_tot = clock() - wtime_tot;

  // report
  printf("\n");
  printf(" wall clock elapsed time (solution) = %f sec\n", ((double) wtime_sol) / CLOCKS_PER_SEC );
  printf(" wall clock elapsed time (total)    = %f sec\n", ((double) wtime_tot) / CLOCKS_PER_SEC );
